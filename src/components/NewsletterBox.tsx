export function NewsletterBox({ compact = false }: { compact?: boolean }) {
  return (
    ...
    >
      <p className="font-display text-xs font-black uppercase tracking-[0.18em] text-[#FF1A1A]">PRESDA Dispatch</p>
      <h2 className={`${compact ? "text-2xl" : "text-4xl sm:text-5xl"} mt-3 font-display font-black uppercase leading-none`}>
      ...
      <form className={`mt-6 grid gap-3 ${compact ? "" : "sm:grid-cols-[1fr_auto]"}`}>
        ...
        />
        <button type="button" className="rounded-lg bg-[#FF1A1A] px-5 py-4 font-display text-xs font-black uppercase tracking-wide text-white transition hover:bg-red-500">
          Subscribe
        </button>
      </form>
    </section>
